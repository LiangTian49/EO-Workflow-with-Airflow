...
def validate_step_data(application_context):
    ...
    """
    assert isinstance(application_context, ApplicationContext),        "Wrong input parameter type for 'application_context', expected ApplicationContext"
    ...
    assert isinstance(application_context.s2_images, Sentinel2Images),        "Wrong type for 's2_images', expected Sentinel2Images"
    assert isinstance(application_context.clc_map_converted_matrix, numpy.ndarray),        "Wrong type for 'clc_map_converted_matrix', expected numpy.ndarray"

    if not configuration.SOURCE_REMOTE:
        assert os.path.exists(application_context.s2_images.path) and                os.path.isdir(application_context.s2_images.path),                "The path for Sentinel2 images doesn't exist or it is not a folder"
    assert isinstance(application_context.input_parameters.s2_samples_image_name, str) and        application_context.input_parameters.s2_samples_image_name is not None and        len(application_context.input_parameters.s2_samples_image_name) > 0,        "The input parameter for the Sentinel2 image for computing the samples is expected to be a string representing "         "a file name"
...
def get_indexes_matrix(matrix, logger):
    """
        ...
        :rtype: numpy.ndarray
    """
    assert isinstance(matrix, numpy.ndarray),        "Wrong type for 'matrix', expected numpy.ndarray"
...
def get_samples_matrix(application_context):
    ...
    if s2_images is not None and         s2_images.name_matrix_map is not None and         application_context.input_parameters.s2_samples_image_name in s2_images.name_matrix_map and         s2_images.name_matrix_map[application_context.input_parameters.s2_samples_image_name] is not None:
...
def get_clc_matrix(application_context):
    """
        Retrieve the Corine Land Cover (CLC) matrix and reshape it 
        :param application_context: The application context
        :type application_context: ApplicationContext
        :return: The samples image
        :rtype: numpy.ndarray
    ...
    # Reshape the clc matrix
    application_context.logger.info("Removing the areas of the CLC which have clouds in the Sentinel2 image")
    clc_map_converted_matrix = application_context.clc_map_converted_matrix.copy()
    #clc_map_converted_matrix_cloud_free[cloud_cover_mask_matrix == 1] = 0
    clc_map_converted_matrix = numpy.reshape(
        clc_map_converted_matrix,
        (clc_map_converted_matrix.shape[0] * clc_map_converted_matrix.shape[1], 1),
        order='F')
...
def main(application_context):
    ...
    # Getting the sample image（s2 image）
    samples_matrix = get_samples_matrix(application_context)

    # Reading the cloud cover image
    converted_clc_map = get_clc_matrix(application_context)
    ...
    # Iterate over all the classes (16 in total)
    for seom_class in configuration.seomClcLegend.seom_classes:
        ...
        clc_indexes_for_class = numpy.where(converted_clc_map[:, 0] == seom_class.class_value)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--id_acquisition", help="Select id of the acquisition for s3", type=int, default=1)
    args = parser.parse_args()
    application_context.id_acquisition = args.id_acquisition
    
    year = "2018"
    path = "/p/project/sdlrs/tian1/lcmap_Netherlands/data/input/"
    #tile_list = ['31UES', '31UET', '31UFS', '31UFT', '31UFU', '31UFV', '31UGS', '31UGU', '31UGV', '31UGT'] # will be imported from the csv file soon
    ...
    if application_context.id_acquisition == 1:
        ...
        #application_context.clc_map_converted_matrix = utils.transform_image_to_matrix(images[0],target_type=configuration.IMAGES_CLC_NUMPY_TYPE)
    ...
    #application_context.input_parameters.s2_samples_image_name = year + "_season1.tif"
    application_context.clc_map_converted_matrix = utils.transform_image_to_matrix(images[0],target_type=configuration.IMAGES_CLC_NUMPY_TYPE)
